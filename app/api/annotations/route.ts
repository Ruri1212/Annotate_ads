...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!annotations || !imageIds || !Array.isArray(annotations) || !Array.isArray(imageIds)) {
    ...
    // 画像メタデータを取得
    const imageMetadataPromises = imageIds.map(async (imageId: string, index: number) => {
      // Base64エンコードされたIDをデコード
      const fileName = Buffer.from(imageId, 'base64').toString('utf-8');
      
      // 画像メタデータ取得APIを呼び出し
      const response = await fetch(`${request.nextUrl.origin}/api/images/metadata?image=${encodeURIComponent(fileName)}`);
      
      if (!response.ok) {
        throw new Error(`画像 ${fileName} のメタデータの取得に失敗しました`);
      }
    ...
    // アノテーションデータをファイルに保存
    const savePath = path.join(process.cwd(), 'public', 'annotations', 'annotation_data.json');
    ...
    console.error('アノテーションデータの保存中にエラーが発生しました:', error);
    return NextResponse.json(

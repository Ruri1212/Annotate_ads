 ...
 */
export const saveAnnotationsData = async (
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'アノテーションの保存中に不明なエラーが発生しました'
    };
  }
};
 ...
 */
export const loadAnnotationsData = async (): Promise<{ [key: string]: Annotation[] }> => {
  try {
    const response = await fetch('/api/annotations');
